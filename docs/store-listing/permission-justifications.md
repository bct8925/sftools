...
| `storage` | Stores OAuth tokens, connection info (instance URLs, labels), user preferences (theme), and cached Salesforce metadata (object/field definitions). Data stays in Chrome storage and is not transmitted externally. |
| `unlimitedStorage` | Allows caching Salesforce object metadata without hitting Chrome's default 10MB storage limit. Large orgs with many custom objects can exceed this limit. Data remains local. |
