...
function validate(scenarios, allTestFiles) {
  ...
  for (const [testId, { file, line }] of scenarios) {
    ...
    if (type === 'integration' && line) {
      ...
      if (line > lineCount) {
        errors.push(`${testId} references \`${file}:${line}\` but file only has ${lineCount} lines`);
      }
    }
  }
  ...
  for (const [type, testFiles] of Object.entries(allTestFiles)) {
    for (const [file, ids] of testFiles) {
      for (const testId of ids) {
        ...
        if (idType !== type) {
          warnings.push(`${testId} (${idType} ID) found in ${type} test file \`${file}\``);
        }

        if (expectedPath && !file.startsWith(expectedPath)) {
          warnings.push(`${testId} in \`${file}\` doesn't match expected path \`${expectedPath}\``);
        }
      }
    }
  }
...
function printReport(scenarios, allTestFiles, { errors, warnings }) {
  ...
  for (const [type, files] of Object.entries(allTestFiles)) {
    console.log(`  - ${type.charAt(0).toUpperCase() + type.slice(1)}: ${files.size} files in tests/${type === 'frontend' ? 'frontend/specs' : type}/`);
  }
  console.log('');
