...
class RecordPage extends HTMLElement {
    ...
    private attachFieldEventListeners(): void {
        ...
        this.fieldsContainer
            .querySelectorAll<HTMLSelectElement>('select.field-input')
            .forEach(select => {
                select.addEventListener('change', () => this.handleFieldChange(select));
            });

        this.fieldsContainer
            .querySelectorAll<HTMLButtonElement>('.field-preview-btn')
            .forEach(btn => {
    ...
    private saveChanges = async (): Promise<void> => {
        ...
        try {
            ...
            this.fieldsContainer
                .querySelectorAll<HTMLElement>('.field-row.modified')
                .forEach(row => {
                    row.classList.remove('modified');
                });
