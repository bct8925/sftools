...
export function FormulaEditor({
    ...
    useEffect(() => {
        ...
        const loadFormula = async () => {
            ...
            try {
                ...
                editorRef.current?.setValue(metadata.formula || '');
                setStatus({ text: '', type: '' });
            } catch (err) {
        ...
        completionDisposableRef.current = monaco.languages.registerCompletionItemProvider('apex', {
            ...
            provideCompletionItems: (model, position) => {
                ...
                if (dotMatch) {
                    ...
                    const relNames = [
                        ...new Set(relationshipFields.map(rf => rf.relationshipName)),
                    ];
    ...
    const handleSave = async () => {
        ...
        const currentFormula = editorRef.current?.getValue() ?? '';

        setIsSaving(true);
        setStatus({ text: 'Saving...', type: '' });

        try {
            await updateFormulaField(fieldMetadata.id, currentFormula, fieldMetadata.metadata);
        ...
        <Modal isOpen={isOpen} onClose={handleClose} className={styles.formulaModal}>
            <div className={styles.modal}>
                ...
                <div className={styles.modalFooter}>
                    <div
                        className={`${styles.modalStatus}${status.type ? ` ${styles[status.type]}` : ''}`}
                    >
                        {status.text}
                    </div>
