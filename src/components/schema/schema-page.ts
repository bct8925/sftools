...
import './schema.css';
import type { SalesforceConnection, SObjectDescribe, FieldDescribe } from '../../types/salesforce';
...
function getCompletionKind(field: FieldDescribe): languages.CompletionItemKind {
    if (field.calculated) {
...
class SchemaPage extends HTMLElement {
    ...
    private allObjects: SObjectDescribe[] = [];
    private filteredObjects: SObjectDescribe[] = [];
    private selectedObject: SObjectDescribe | null = null;
    private allFields: FieldDescribe[] = [];
    private filteredFields: FieldDescribe[] = [];

    // DOM references
    private objectFilterEl!: HTMLInputElement;
    private objectCountEl!: HTMLElement;
    private objectsListEl!: HTMLElement;
    private refreshObjectsBtnEl!: HTMLButtonElement;
    private fieldsPanelEl!: HTMLElement;
    private selectedObjectLabelEl!: HTMLElement;
    private selectedObjectNameEl!: HTMLElement;
    private fieldFilterEl!: HTMLInputElement;
    private fieldsListEl!: HTMLElement;
    private refreshFieldsBtnEl!: HTMLButtonElement;
    private closeFieldsBtnEl!: HTMLButtonElement;

    // Modal references
    private formulaModalEl!: HTMLElement;
    private formulaEditorEl!: MonacoEditorElement;
    private modalFieldInfoEl!: HTMLElement;
    private modalStatusEl!: HTMLElement;
    private modalSaveBtnEl!: HTMLButtonElement;
    private modalCancelBtnEl!: HTMLButtonElement;
    private modalCloseBtnEl!: HTMLButtonElement;

    // Current formula field being edited
    private currentFormulaField: CurrentFormulaField | null = null;
    ...
    private initElements(): void {
        this.objectFilterEl = this.querySelector<HTMLInputElement>('#objectFilter')!;
        this.objectCountEl = this.querySelector<HTMLElement>('#objectCount')!;
        this.objectsListEl = this.querySelector<HTMLElement>('#objectsList')!;
        this.refreshObjectsBtnEl = this.querySelector<HTMLButtonElement>('#refreshObjectsBtn')!;
        this.fieldsPanelEl = this.querySelector<HTMLElement>('#fieldsPanel')!;
        this.selectedObjectLabelEl = this.querySelector<HTMLElement>('#selectedObjectLabel')!;
        this.selectedObjectNameEl = this.querySelector<HTMLElement>('#selectedObjectName')!;
        this.fieldFilterEl = this.querySelector<HTMLInputElement>('#fieldFilter')!;
        this.fieldsListEl = this.querySelector<HTMLElement>('#fieldsList')!;
        this.refreshFieldsBtnEl = this.querySelector<HTMLButtonElement>('#refreshFieldsBtn')!;
        this.closeFieldsBtnEl = this.querySelector<HTMLButtonElement>('#closeFieldsBtn')!;

        // Modal elements
        this.formulaModalEl = this.querySelector<HTMLElement>('#formulaModal')!;
        this.formulaEditorEl = this.querySelector<MonacoEditorElement>('#formulaEditor')!;
        this.modalFieldInfoEl = this.querySelector<HTMLElement>('#modalFieldInfo')!;
        this.modalStatusEl = this.querySelector<HTMLElement>('#modalStatus')!;
        this.modalSaveBtnEl = this.querySelector<HTMLButtonElement>('#modalSaveBtn')!;
        this.modalCancelBtnEl = this.querySelector<HTMLButtonElement>('#modalCancelBtn')!;
        this.modalCloseBtnEl = this.querySelector<HTMLButtonElement>('#modalCloseBtn')!;
    }

    private attachEventListeners(): void {
        this.objectFilterEl.addEventListener('input', this.handleObjectFilterInput);
        this.refreshObjectsBtnEl.addEventListener('click', this.refreshObjects);
        this.fieldFilterEl.addEventListener('input', this.handleFieldFilterInput);
        ...
        this.formulaModalEl.addEventListener('click', this.handleFormulaModalClick);

        // Close any open field menus when clicking outside (store bound handler for cleanup)
        this.boundDocClickHandler = (e: MouseEvent) => {
            if (
                !(e.target as HTMLElement).closest('.field-menu-button') &&
                !(e.target as HTMLElement).closest('.field-menu')
            ) {
    ...
    private handleObjectFilterInput = (e: Event): void => {
        this.filterObjects((e.target as HTMLInputElement).value);
    };

    private filterObjects(searchTerm: string): void {
    ...
    private renderObjects(): void {
        ...
        this.objectsListEl.querySelectorAll<HTMLElement>('.object-item').forEach(item => {
    ...
    private async selectObject(objectName: string): Promise<void> {
        // Update selected state in UI
        this.objectsListEl.querySelectorAll('.object-item').forEach(item => {
            item.classList.toggle('selected', (item as HTMLElement).dataset.name === objectName);
        });
        ...
        const objectsPanel = this.objectsListEl.closest<HTMLElement>('.objects-panel')!;
    ...
    private navigateToObject(objectName: string): void {
        ...
        const objectItem = this.objectsListEl.querySelector<HTMLElement>(
    ...
    private async loadFields(objectName: string, bypassCache = false): Promise<void> {
        ...
        try {
            ...
            this.fieldsListEl.innerHTML = `
                <div class="error-container">
                    <p class="error-message">${escapeHtml((error as Error).message)}</p>
    ...
    private refreshObjects = async (): Promise<void> => {
    ...
    private refreshFields = async (): Promise<void> => {
    ...
    private handleFieldFilterInput = (e: Event): void => {
        this.filterFields((e.target as HTMLInputElement).value);
    };

    private filterFields(searchTerm: string): void {
    ...
    private renderFields(): void {
        ...
        this.fieldsListEl.innerHTML = fields
            .map(field => {
                ...
                const isFormulaField =
                    field.calculated &&
                    (field as FieldDescribe & { calculatedFormula?: string }).calculatedFormula;
        ...
        this.fieldsListEl.querySelectorAll<HTMLAnchorElement>('.reference-link').forEach(link => {
        ...
        this.fieldsListEl
            .querySelectorAll<HTMLButtonElement>('.field-menu-button')
            .forEach(button => {
        ...
        this.fieldsListEl.querySelectorAll<HTMLElement>('.field-menu-item').forEach(item => {
            item.addEventListener('click', e => {
                e.stopPropagation();
                const { action, fieldName } = item.dataset;

                if (action === 'edit' && fieldName) {
    ...
    private getFieldTypeDisplay(field: FieldDescribe): {
        text: string;
        isReference: boolean;
        referenceTo?: string[];
    } {
    ...
    private closeFieldsPanel = (): void => {
        this.fieldsPanelEl.style.display = 'none';
        this.objectsListEl.closest<HTMLElement>('.objects-panel')!.classList.remove('with-fields');
        this.objectsListEl.querySelectorAll('.object-item').forEach(item => {
    ...
    // Field menu methods
    private toggleFieldMenu(fieldName: string): void {
        const menu = this.fieldsListEl.querySelector<HTMLElement>(
            `.field-menu[data-field-name="${fieldName}"]`
        );
        if (!menu) return;
    ...
    private closeAllFieldMenus(): void {
    ...
    private async openFormulaEditor(field: FieldDescribe): Promise<void> {
        ...
        void this.loadRelationshipFields();
        ...
        try {
            ...
            this.modalStatusEl.textContent = `Error loading formula: ${(error as Error).message}`;
            this.modalStatusEl.className = 'modal-status error';
        }
    }

    private async loadRelationshipFields(): Promise<void> {
    ...
    private closeFormulaModal = (): void => {
    ...
    private saveFormula = async (): Promise<void> => {
        ...
        try {
            await updateFormulaField(
                ...
                newFormula,
                this.currentFormulaField.metadata as Record<string, unknown>
            );
            ...
            setTimeout(() => {
                ...
                void this.loadFields(this.selectedObject!.name);
            }, 1500);
        } catch (error) {
            this.modalStatusEl.textContent = `Error saving: ${(error as Error).message}`;
            this.modalStatusEl.className = 'modal-status error';
            this.modalSaveBtnEl.disabled = false;
        }
    };

    private handleFormulaModalClick = (e: MouseEvent): void => {
