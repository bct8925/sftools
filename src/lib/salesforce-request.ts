...
export async function salesforceRequest<T = unknown>(
  ...
  if (!response.success && response.status !== 404) {
    ...
    let error: SalesforceErrorResponse | SalesforceErrorResponse[] = { message: response.statusText };
    if (response.data) {
      try {
        error = JSON.parse(response.data) as SalesforceErrorResponse | SalesforceErrorResponse[];
      } catch {
