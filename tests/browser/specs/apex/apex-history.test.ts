 ...
 */
describe('Apex History (A-F-008, A-F-009, A-F-010)', () => {
...
System.debug('Result: ' + result);`;
    ...
    it('can save to, load from, and delete from history', async () => {
        ...
        const historyCount = await apexTab.getHistoryCount();
        expect(historyCount).toBeGreaterThan(0);

        const historyItems = await apexTab.getHistoryItems();
        ...
        await apexTab.openHistory();
        const beforeDeleteCount = await apexTab.getHistoryCount();
        ...
        await apexTab.closeHistory();
        await apexTab.openHistory();
        const afterDeleteCount = await apexTab.getHistoryCount();
