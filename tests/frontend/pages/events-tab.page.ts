...
export class EventsTabPage extends BasePage {
  ...
  async waitForChannelsLoaded(): Promise<void> {
    ...
    await this.page.waitForSelector('events-tab .event-channel-select option[value="/event/Order_Event__e"]', {
      state: 'attached',
      timeout: 10000
    });
  }
  ...
  async waitForPublishChannelsLoaded(): Promise<void> {
    ...
    await this.page.waitForSelector('events-tab .event-publish-channel option[value="Order_Event__e"]', {
      state: 'attached',
      timeout: 10000
    });
  }
