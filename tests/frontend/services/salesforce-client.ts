...
export class SalesforceClient {
  ...
  async createContact(data: { LastName: string; AccountId?: string; [key: string]: unknown }): Promise<string> {
  ...
  async cleanupAll(): Promise<void> {
    ...
    for (const record of this.createdRecords.reverse()) {
      try {
        ...
        console.warn(`  ⚠️ Failed to delete ${record.type}/${record.id}: ${(e as Error).message}`);
      }
    }
    this.createdRecords = [];
  }
  ...
  // Get a single record
  async getRecord<T = Record<string, unknown>>(objectType: string, id: string, fields?: string[]): Promise<T> {
