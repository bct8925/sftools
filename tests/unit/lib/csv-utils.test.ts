...
describe('getValueByPath', () => {
  ...
  it('returns simple field value', () => {
    const record = { Id: '001xx', Name: 'Test Account', attributes: { type: 'Account', url: '' } };
...
describe('formatCellValue', () => {
  ...
  it('formats subquery with records array', () => {
    ...
    expect(formatCellValue(subquery, { path: 'Contacts', isSubquery: true })).toBe('[2 records]');
  });

  it('formats subquery using records length if totalSize missing', () => {
    const subquery = { records: [{ Id: '001' }, { Id: '002' }, { Id: '003' }], done: true };
    expect(formatCellValue(subquery, { path: 'Contacts', isSubquery: true })).toBe('[3 records]');
  });
